...
public class PageService {
    ...
    private FamilyPageView programHub(String requestBaseUrl) {
        ...
        return new FamilyPageView(
                ...
                "This hub owns the official-looking questions that users usually type first after the inspection report arrives.",
                List.of(
                        "Do not treat the report as approval.",
                        "Do not treat support-state issues like Group 5 or RFI as project-choice answers.",
                        "Do not jump to quote shopping before the first project is clear."),
                "Core decision routes",
                "These are the indexable routes that should own the main post-report search intents.",
    ...
    private FamilyPageView guideHub(String requestBaseUrl) {
        ...
        return new FamilyPageView(
                ...
                "These pages should own helper-style search intents and then hand the user back to the core route that makes the next decision.",
                List.of(
                        "Do not let a guide replace the route that actually owns the decision.",
                        "Do not let two guides answer the same search intent.",
                        "Do not turn checklist pages into generic FAQ clones."),
                "Decision guides",
                "Each guide should answer one supporting question cleanly and then return the user to a core route.",
                detailCardsForPaths(List.of(
    ...
    private FamilyPageView improvementHub(String requestBaseUrl) {
        ...
        return new FamilyPageView(
                ...
                "These pages should own improvement-specific search intents, especially where roof-related work can easily drift into the wrong project path.",
                List.of(
                        "Do not let every roof recommendation collapse into generic reroof logic.",
                        "Do not assume attached homes follow detached-home scope.",
                        "Do not let improvement pages all compete for the same eligibility question."),
                "Improvement routes",
                "Each route should own one improvement category and a distinct question the homeowner is trying to answer.",
                detailCardsForPaths(List.of(
